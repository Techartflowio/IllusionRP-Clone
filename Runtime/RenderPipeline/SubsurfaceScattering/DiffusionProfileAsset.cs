...
namespace Illusion.Rendering
{
    ...
    /// </summary>
    [CreateAssetMenu(fileName = "DiffusionProfile", menuName = "Rendering/Diffusion Profile")]
    public class DiffusionProfileAsset : ScriptableObject
    {
        ...
        [NonSerialized] 
        internal Vector4 worldScaleAndFilterRadiusAndThicknessRemap; // X = meters per world unit, Y = filter radius (in mm), Z = remap start, W = end - start
        
        [NonSerialized] 
        internal Vector4 shapeParamAndMaxScatterDist;                // RGB = S = 1 / D, A = d = RgbMax(D)
        
        [NonSerialized] 
        internal Vector4 transmissionTintAndFresnel0;                // RGB = color, A = fresnel0
        
        [NonSerialized] 
        internal Vector4 disabledTransmissionTintAndFresnel0;        // RGB = black, A = fresnel0 - For debug to remove the transmission
        ...
        internal void UpdateCache()
        {
            ...
            transmissionTintAndFresnel0 = new Vector4(profile.transmissionTint.r, profile.transmissionTint.g, profile.transmissionTint.b, fresnel0);
